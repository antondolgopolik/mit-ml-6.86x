...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    fn = np.vectorize(hinge_loss_single, signature='(n),(),(n),()->()')
    ls = fn(feature_matrix, labels, theta, theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    dim = feature_matrix.shape[1]
    theta, theta_0 = np.zeros(dim), 0
    sum_theta, sum_theta_0 = theta.copy(), theta_0
    for t in range(T):
        for i in get_order(nsamples):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            sum_theta += theta
    ...
    for t in range(T):
        for i in get_order(nsamples):
            ...
            theta, theta_0 = pegasos_single_step_update(
                feature_matrix[i], labels[i], L, 1 / math.sqrt(steps),
                theta, theta_0
            )
    return theta, theta_0
...
def classifier_accuracy(
    ...
    train_ys = classify(train_feature_matrix, theta, theta_0)
    val_ys = classify(val_feature_matrix, theta, theta_0)
    train_correct = np.sum(train_ys == train_labels)
    val_correct = np.sum(val_ys == val_labels)
    return train_correct / len(train_labels), val_correct / len(val_labels)
